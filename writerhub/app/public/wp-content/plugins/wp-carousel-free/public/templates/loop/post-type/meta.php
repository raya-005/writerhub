...
if ( $show_post_date || $show_post_author || $show_post_comment ) {
	...
	<ul class="wpcp-post-meta">
		<?php if ( $show_post_author ) { ?>
			...
			printf( esc_html__( 'By %s', 'wp-carousel-free' ), esc_html( get_the_author() ) );
			?>
			</a></li>
		<?php } ?>
		<?php if ( $show_post_date ) { ?>
			<li><time class="entry-date published updated" datetime="<?php echo esc_attr( get_the_date( 'c' ) ); ?>">
				<?php
				// Translators: %s is the post date.
				printf( esc_html__( 'On %s', 'wp-carousel-free' ), esc_html( get_the_date() ) );
				?>
			</time></li>
			<?php
		}
